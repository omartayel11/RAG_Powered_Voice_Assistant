...
db = client.get_database_by_api_endpoint(
  "https://e9a6f230-db53-4451-a94b-515ffae8b800-us-east-2.apps.astra.datastax.com"
)
...
# Define the maximum token size for embeddings (for example, 512 tokens)
...
# Function to split the recipe into chunks and limit it to MAX_TOKENS
def split_and_limit_recipe(recipe, max_tokens=MAX_TOKENS):
    # Split the recipe by spaces (or any other logic to split sentences/paragraphs)
    words = recipe.split(" ")  # Split by words; you can change this logic based on your needs
    chunk = " ".join(words[:max_tokens])  # Get only the first part up to the max tokens
    return chunk

# Insert the recipe data into Astra DB
for doc in recipes_data:
    full_recipe = f"{doc[0]}\n{doc[1]}\n{doc[2]}\n{doc[3]}"
    
    # Limit the recipe to the specified number of tokens (words in this case)
    limited_recipe = split_and_limit_recipe(full_recipe)
    
    # Check if the recipe exceeds the token limit and adjust accordingly
    if len(limited_recipe.split()) > MAX_TOKENS:
        print(f"Warning: Recipe '{doc[0]}' exceeds the token limit. Trimming to {MAX_TOKENS} tokens.")
    
    # Prepare the document for insertion into Astra DB
    document = {
        "id": f"{doc[5]}_full_recipe",  # Unique ID for each recipe
        "name": doc[0],  # Recipe name
        "description": doc[1],  # Recipe description
        "ingredients": doc[2],  # Ingredients
        "instructions": doc[3],  # Instructions
        "category": doc[4],  # Category
        "full_recipe": full_recipe,  # Store the full recipe text
        "$vectorize": doc[0]  # Vectorize only the first part of the recipe
    }

    # Insert the document into the collection
    collection.insert_one(document)

print("Data has been successfully added to the 'firstcollection' with vector embeddings for part of the recipe.")
